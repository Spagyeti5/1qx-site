...
import { inventionQuestions } from '../data/quizQuestions';
...
function QuizApp() {
  ...
  const correctSnarkyComments = [
    ...
    "Time traveler alert!",
    "You really know your inventions!",
  ...
  useEffect(() => {
    const shuffledQuestions = [...inventionQuestions]
  ...
  const restartQuiz = () => {
    const shuffled = [...inventionQuestions]
      .sort(() => 0.5 - Math.random())
      .slice(0, 10)
      .map(q => ({
        ...q,
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-orange-50 p-8">
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold mb-2">Guess the Invention Year!</h2>
        <p className="text-gray-700">Can you guess when famous inventions were created? Pick the correct year!</p>
      </div>
